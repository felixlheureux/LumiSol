...
Ingest: Query provincial indices for LiDAR tiles.

Stratify: Select a balanced mix of Urban, Suburban, and Rural tiles.

Download: Fetch massive GeoTIFFs (Height Maps) on-demand.

Engineer Features: Convert raw Height into a 3-Channel Geometric Tensor.

Synthesize Labels: Burn government vector footprints into raster masks.

Augment: Apply "Negative Erosion" to fix labeling errors automatically.

🚀 The Scripts (Explained)

1. backend/scripts/generate_mask2former_data.py

"The Engine" - This script creates the dataset.
...
Stratified Sampling: It doesn't just pick random tiles. It calculates building density (buildings/km²) and forces a distribution of 33% High Density (Downtown), 33% Medium (Suburbs), and 33% Low (Rural). This prevents the "Empty Forest" bias.
...
3. backend/scripts/test_single_pair.py
...
Locates height tiles across Quebec.

GeoJSON

Building Vectors

Données Québec (Référentiel)
...
Raw canopy height (Surface - Ground).
...
Channel 1: Normalized Height



Channel 2: Slope (Gradient)



Channel 3: Roughness (Laplacian)
